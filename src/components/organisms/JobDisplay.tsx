...
export default function JobDisplay() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 w-full">
      <div className="col-span-1 flex flex-col items-start justify-start border-l-2 border-slate-800/50 mb-4">
        {jobs.map((job, index) => (
          <Button
            key={index}
            variant="ghost"
            className={`gap-4 sm:px-10 md:px-0 py-1 md:py-2 grid grid-cols-3 w-full rounded-none hover:border-l-2 hover:border-indigo-600 group ${
              job.company === focused ? "border-l-2 border-indigo-600" : ""
            }`}
            onClick={() => setFocused(job.company)}
          >
            <span
              className={`col-span-1 font-light text-xs text-left md:text-center group-hover:text-indigo-600 ${
                job.company === focused
                  ? "text-indigo-600"
                  : "text-slate-800/80"
              }`}
            ...
            <span
              className={`text-left col-span-2 text-xs sm:text-sm group-hover:text-indigo-600
                  ${
                    job.company === focused
                      ? "text-indigo-600"
                      : "text-slate-800/90"
      ...
      <div className="col-span-1 md:col-span-3 border border-slate-800/70 bg-white md:bg-transparent md:border-none rounded-xl p-4 md:p-0 md:mx-4">
        {focused === "Vucar" && <Vucar />}
  ...
  return (
    <div className="flex flex-col items-start justify-start gap-4 w-full">
      <div className="flex flex-col items-start justify-start gap-1">
        ...
        <span className="text-[13px] md:text-[14px] text-slate-800/80">
          May — August 2024 &nbsp;&nbsp;|&nbsp;&nbsp; Onsite in Ho Chi Minh City
        </span>
      </div>
      <p className="text-gray-600/80 text-[13px] md:text-[14px] text-justify">
      ...
      <div className="text-gray-600/80 text-[13px] md:text-[14px] gap-2 grid grid-cols-12 w-full">
        <Check className="text-indigo-600 size-4 md:size-5 col-span-1 mt-1" />
